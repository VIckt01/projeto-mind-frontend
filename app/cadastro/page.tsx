...
export default function Cadastro() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(
        err.response?.data?.error || err.message || "Erro ao efetuar cadastro.",
      );
    } finally {
      setLoading(false);
    }
    ...
    <div className="w-full min-h-screen bg-[#0B0E13] text-white flex flex-col justify-between antialiased font-sans select-none">
      ...
      <main className="w-full max-w-[1440px] mx-auto px-4 py-16 flex flex-col items-center gap-10 flex-1 justify-center">
        ...
        <div className="w-full bg-[#14181F] border border-zinc-900/80 rounded-xl p-8 max-w-[440px] shadow-2xl">
          {error && (
            <div className="mb-4 bg-red-500/10 border border-red-500/20 text-red-400 text-xs p-3 rounded text-center">
              {error}
            </div>
          )}
          {success && (
            <div className="mb-4 bg-teal-500/10 border border-teal-500/20 text-teal-400 text-xs p-3 rounded text-center">
              {success}
            </div>
          )}
          ...
          <form onSubmit={handleRegister} className="flex flex-col gap-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-cyan-400 hover:bg-cyan-300 text-zinc-950 font-bold py-3 rounded text-xs uppercase tracking-wider transition-all cursor-pointer mt-2 disabled:opacity-50"
            >
