...
export default function DetalheArtigoPage() {
  ...
  useEffect(() => {
    async function carregarArtigo() {
      ...
      if (user) {
        setArticleLiked(
          localStorage.getItem(`techblog_like_${user.id}_${data.id}`) ===
            "true",
        );
        setArticleSaved(
          localStorage.getItem(`techblog_save_${user.id}_${data.id}`) ===
            "true",
        );
      }
      ...
      leiturasArray = leiturasArray.filter(
        (l: any) => String(l.id) !== String(data.id),
      );
      ...
      localStorage.setItem(
        "techblog_leituras",
        JSON.stringify(leiturasArray.slice(0, 4)),
      );
  ...
  const handleLikeArticle = () => {
    ...
    setArtigo((prev) =>
      prev ? { ...prev, likes: prev.likes + (novoStatus ? 1 : -1) } : null,
    );
    localStorage.setItem(
      `techblog_like_${user.id}_${artigo.id}`,
      novoStatus.toString(),
    );
  };
  ...
  const handleSaveArticle = () => {
    ...
    localStorage.setItem(
      `techblog_save_${user.id}_${artigo.id}`,
      novoStatus.toString(),
    );
  };
  ...
  return (
    <div className="w-full min-h-screen bg-[#070a13] text-zinc-100 antialiased font-sans flex flex-col items-center">
      <div className="w-full max-w-3xl px-6 py-12 flex flex-col">
        <Link
          href="/artigos"
          className="text-xs text-zinc-500 hover:text-cyan-400 mb-8 inline-block"
        >
        ...
        <div className="w-full flex items-center justify-between border-b border-zinc-900 pb-4 mt-6">
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col text-[11px]">
              <span className="font-bold">{artigo.author}</span>
              <span className="text-zinc-500">
                {artigo.date} • {artigo.readTime}
              </span>
            </div>
          </div>
          ...
          <div className="flex gap-4">
            ...
            <button
              ...
              className={`cursor-pointer transition-transform active:scale-90 ${articleLiked ? "text-red-500" : "text-zinc-500 hover:text-white"}`}
              title="Curtir Artigo"
            >
              {articleLiked ? (
                <svg
            ...
            <button
              ...
              className={`cursor-pointer transition-transform active:scale-90 ${articleSaved ? "text-cyan-400" : "text-zinc-500 hover:text-white"}`}
              title="Salvar Artigo"
            >
              {articleSaved ? (
                <svg
        ...
        <div className="text-zinc-300 text-sm leading-relaxed whitespace-pre-line text-justify">
          {artigo.content}
        </div>

        <section className="border-t border-zinc-900 pt-6 mt-10">
          <h2 className="text-xs font-bold text-white mb-5">
            Comentários ({artigo.commentsCount})
          </h2>
          ...
          {user ? (
            ...
            >
              ...
              <div className="flex justify-end mt-2">
                <button
                  type="submit"
                  className="bg-cyan-400 text-zinc-950 font-bold px-4 py-1.5 rounded-md text-xs cursor-pointer"
                >
                  Comentar
                </button>
              </div>
            ...
            <div className="text-center p-6 border border-zinc-900 rounded-xl mb-8">
              Faça{" "}
              ...
              <div key={comment.id} className="flex flex-col gap-3">
                <div className="bg-[#0d111e]/20 border border-zinc-900/60 rounded-xl p-4">
                  <div className="flex items-center gap-2 mb-2">
                    <div className="w-7 h-7 bg-[#14181F] rounded-full flex items-center justify-center text-[10px] font-bold">
                  ...
                  <div className="flex gap-4 mt-2">
                    <button
                      ...
                      className={`flex items-center gap-1.5 text-[10px] cursor-pointer ${comment.userLiked ? "text-red-400" : "text-zinc-500"}`}
                    >
                    ...
                    {user && (
                      <button
                        ...
                        className="text-[10px] text-zinc-500 hover:text-cyan-400 cursor-pointer"
                      >
                        Responder
                      </button>
                    )}
                ...
                {replyingTo === comment.id && (
                  <form
                    onSubmit={(e) => handleEnviarResposta(e, comment.id)}
                    className="ml-8 bg-[#0B0E13] border border-cyan-900/30 rounded-lg p-3"
                  >
                    <textarea
                      value={replyContent}
                    ...
                    <button
                      type="submit"
                      className="bg-cyan-500/10 text-cyan-400 px-3 py-1 text-[10px] rounded mt-2 cursor-pointer"
                    >
                      Enviar
                    </button>
                  </form>
                ...
                {comment.replies?.map((reply) => (
                  <div
                    key={reply.id}
                    className="ml-8 bg-[#0B0E13] border border-zinc-900/40 rounded-lg p-3"
                  >
